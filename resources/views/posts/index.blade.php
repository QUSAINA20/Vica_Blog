                ...
                <div class="col-md-8">
                    {{-- <form action="{{ route('posts.index') }}" method="get">
                        <div class="input-group">
                            <input type="text" name="query" class="form-control rounded-0 border-end-0"
                                placeholder="Search for posts" value="{{ $query ?? '' }}">
                            <select name="category" class="form-control rounded-0 border-end-0">
                                <option value="">All categories</option>
                                @foreach ($categories as $category)
                                    <option value="{{ $category->slug }}"
                                        {{ $categorySlug == $category->slug ? 'selected' : '' }}>{{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-4 mb-3">
                        <div class="card h-100">
                            ...
                            <div class="card-footer">
                                <small class="text-muted">{{ $post->created_at->diffForHumans() }} by
